...
def download():
    while True:
        try:
            url = download_files.get(timeout=10)
        except queue.Empty:
            return
        else:
            ...
            cmd = "curl --silent " + url + " --output " + download_file
            print("")
            print(cmd)
            print("")
            retval = os.system(cmd)
            if retval != 0:
                download_files.put(url)
                continue

            if args.decompress_dir is not None:
                cmd = "unzip -qq " + download_file + " -d " + args.decompress_dir
                print("")
                print(cmd)
                print("")
                retval = os.system(cmd)
...
def delete_unnecessary():
    while True:
        # excluding_files = ["*.position.hdf5", "normal_cam.hdf5", "*.color.hdf5", "*.depth_meters.hdf5"]
        excluding_files = ["*.position.hdf5", "*.normal_cam.hdf5", "*.color.hdf5", "*.depth_meters.hdf5", "metadata_cameras.csv", "*.render_entity_id.hdf5"]
        cmd = f'find {args.decompress_dir} -type f -not -name "metadata_scene.csv" -not -name "camera_keyframe_*.hdf5"'
        for file in excluding_files:
            cmd = cmd + " -not -name " + f"'{file}'"

        cmd = cmd + " -delete"
        print("Deleting all unnecessary files")
        # print(cmd)
        os.system(cmd)
        time.sleep(10*60)
...
# cmd = f"rm -r {args.decompress_dir}"
# os.system(cmd)
...
processes = [multiprocessing.Process(target=download) for x in range(1, 3)]
delete_process = multiprocessing.Process(target=delete_unnecessary)
delete_process.start()
[p.start() for p in processes]
[p.join() for p in processes]
delete_process.terminate()
# copy intrinsic and split file
cmd = "cp camera_intrinsics.csv hypersim_split.csv " + args.decompress_dir
os.system(cmd)
